...
def generate_fstab(
    ...
    lines = [
        "# /etc/fstab - generado por essora-installer\n",
    ...
    if efi_dev:
        ...
        lines.append(
            f"UUID={efi_uuid}  /boot/efi  {efi_type}  umask=0077  0  1\n"
        )
...
def verify_disk_space(root_part: str, log: LogFn):
    try:
        ...
        if avail < needed:
            raise RuntimeError(
                f"Espacio insuficiente en {root_part}: "
...
def apply_keyboard_settings(
    ...
    with open(kbd_path, "w", encoding="utf-8") as f:
        f.write("# /etc/default/keyboard - generado por essora-installer\n")
...
def deploy_grub(root_mount: str, grub_disk: str, uefi: bool, log: LogFn):
    ...
    if uefi:
        ...
        run_in_target(
            root_mount,
            f"grub-install --target=i386-pc {shlex.quote(grub_disk)} --recheck",
            log,
        )
...
def do_install(plan: InstallPlan, log: LogFn, progress: Callable[[int], None]):
    ...
    if plan.format_root:
        ...
        prepare_partition(plan.root_part, plan.root_fstype, ROOT_LABEL, log)
    ...
    generate_fstab(root_mount, plan.root_part, plan.efi_part, plan.home_part, log)
    ...
    try:
        ...
        if plan.install_grub:
            ...
            deploy_grub(root_mount, plan.grub_disk, plan.uefi, log)
...
def _configure_slim_in_target(
    ...
    log(f"  [post] Configurando SLiM: default_user={username}, auto_login={'yes' if autologin else 'no'}")
...
def _configure_plasma_locale(
            ...
            if os.path.isfile(xsession):
                try:
                    ...
                    if "startkde" in content:
                        kde_detected = True
                        log(f"  [plasma] KDE detectado en /home/{user_dir}/.xsession")
                        break
                except Exception:
                    pass
    ...
    try:
        ...
        log(f"  [plasma] Escrito: /home/{username}/.config/plasma-localerc")
    except Exception as e:
        log(f"  [plasma] WARNING: could not write for user {username}: {e}")

    log(f"  [plasma] plasma-localerc configurado: LANG={lang}, LANGUAGE={lang_code}")
